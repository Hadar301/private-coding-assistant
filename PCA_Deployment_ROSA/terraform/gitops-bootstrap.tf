...
# ════════════════════════════════════════════════
# This installs the OpenShift GitOps operator and creates the
# root App-of-Apps that manages everything else on the cluster.
...
resource "null_resource" "install_gitops_operator" {
  ...
  depends_on = [
    rhcs_cluster_wait.wait,
    null_resource.grant_cluster_admin
  ]
}
